...
const EmployeeProfile = () => {
  const username = localStorage.getItem("loggedInUser"); // saved during login
    ...
    const fetchProfile = async () => {
      ...
      try {
        const res = await fetch(
          `${SHEET_API_URL}?action=getEmployee&username=${encodeURIComponent(
            username
          )}`
        );
  ...
  if (!profile)
    return (
      <p style={{ textAlign: "center" }}>
        No profile found for user: <b>{username}</b>
      </p>
    );
  ...
  <div className="profile-content">
    <div className="profile-grid">
      ...
      <div className="profile-field">
        ...
        <span>{new Date(profile.joiningDate).toLocaleDateString("en-GB")}</span>
      </div>
    </div>
